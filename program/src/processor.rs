...
use {
    crate::{
        ...
        error::SwapError,
        instruction::{
            CreatePool, DepositAllTokenTypes, DepositSingleTokenTypeExactAmountIn, Initialize,
            Swap, SwapExactOut, SwapInstruction, WithdrawAllTokenTypes,
...
impl Processor {
    ...
    /// validation + initial-LP-mint + pack as [`process_initialize`] (the vaults
    ...
    pub fn process_create_pool(
        ...
        // Create the fee + destination LP token accounts INSIDE the instruction —
        // they are token accounts of the LP mint just created, so the caller can't
        // pre-make them. Both are program-signed PDAs; InitializeAccount3 sets each
        // account's owner (fee → pool, so fee.owner != authority as Initialize
        // requires; destination → creator, so the creator holds the initial LP).
        let account_space = Account::LEN;
        let account_rent = rent.minimum_balance(account_space);
        for (info, seed_prefix, owner_key) in [
            (fee_account_info, b"cp_fee".as_slice(), pool_info.key),
            (destination_info, b"cp_dest".as_slice(), payer_info.key),
        ] {
            let (expected, bump) =
                Pubkey::find_program_address(&[seed_prefix, pool_info.key.as_ref()], program_id);
            if expected != *info.key {
                return Err(SwapError::InvalidProgramAddress.into());
            }
            let signer_seeds: &[&[u8]] = &[seed_prefix, pool_info.key.as_ref(), &[bump]];
            invoke_signed(
                &system_instruction::create_account(
                    payer_info.key,
                    info.key,
            ...
            invoke(
                ...
                &[info.clone(), lp_mint_info.clone()],
            )?;
        }

        // Delegate to the SAME validation + initial-LP-mint + pack as Initialize.
        // The vaults must already be funded by the caller (identical to Initialize).
        let init_accounts = [
            pool_info.clone(),
            authority_info.clone(),
            vault_a_info.clone(),
        ...
        Self::process_initialize(program_id, fees, swap_curve, &init_accounts, swap_constraints)
    }
    ...
    use {
        ...
        crate::{
            ...
            instruction::{
                ...
                swap, swap_exact_out, withdraw_all_token_types,
