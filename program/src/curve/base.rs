...
impl SwapCurve {
    ...
    /// Exact-output swap: given a desired amount of destination token, compute
    /// the total source token the swapper must pay (curve input grossed up for
    /// fees). The destination amount is delivered exactly; the input is rounded
    /// up and fees are charged on top, so the pool value never decreases.
    pub fn swap_for_exact_out(
        ...
        // Pre-fee input that must enter the pool to yield the exact output.
        let SwapWithoutFeesResult {
            source_amount_swapped: pool_input,
            destination_amount_swapped,
        } = self.calculator.swap_to_exact_out(
            destination_amount,
            swap_source_amount,
            swap_destination_amount,
            trade_direction,
        ...
        // Gross the pool input up so that, after fees are deducted, at least
        // `pool_input` still reaches the curve. `pre_trading_fee_amount`
        // inverts the combined trade + owner fee.
        let source_amount_swapped = fees.pre_trading_fee_amount(pool_input)?;

        // Recompute the fee split from the gross amount, consistent with the
        // exact-in path (`swap`).
...
mod test {
    ...
    #[test]
    fn constant_product_swap_for_exact_out() {
        let fees = Fees {
        ...
        let r = swap_curve
            ...
            .unwrap();
        // output is delivered exactly
        assert_eq!(r.destination_amount_swapped, want_out);
        assert_eq!(r.new_swap_destination_amount, swap_destination_amount - want_out);
        assert_eq!(r.new_swap_source_amount, swap_source_amount + r.source_amount_swapped);
        // the pool never loses value
        assert!(
            r.new_swap_source_amount * r.new_swap_destination_amount
                >= swap_source_amount * swap_destination_amount
        );
        // fees are charged on top of the required input
        assert!(r.source_amount_swapped > 0);
        assert!(r.trade_fee + r.owner_fee > 0);
    }

    #[test]
    fn exact_out_no_fee_roundtrips_with_exact_in() {
        // With no fees, asking exact-out for D then feeding the returned input
        // back through exact-in must return at least D.
        let fees = Fees::default();
        let swap_source_amount: u128 = 1_000;
        let swap_destination_amount: u128 = 50_000;
        let want_out: u128 = 4_545; // exact-in(100) yields 4545 in the no-fee case
        let swap_curve = SwapCurve {
            curve_type: CurveType::ConstantProduct,
            calculator: Arc::new(ConstantProductCurve),
        };
        ...
        let back = swap_curve
            ...
            .unwrap();
        assert!(
            back.destination_amount_swapped >= want_out,
            "exact-in of {} yielded {} < requested {}",
            r.source_amount_swapped,
            back.destination_amount_swapped,
            want_out
        );
    }
