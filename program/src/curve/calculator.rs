...
pub trait CurveCalculator: Debug + DynPack {
    ...
    /// Calculate how much source token is required to receive an exact amount
    /// of destination token (exact-output swap).
    ///
    /// The default implementation returns `None` — curves opt in by overriding.
    /// Callers that receive `None` must treat exact-output as unsupported for
    /// that curve and revert.
