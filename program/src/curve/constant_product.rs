...
/// The constant product exact-output calculation: given a desired amount of
/// destination token, compute the amount of source token required.
///
/// The input is rounded UP so the invariant never decreases (the pool never
/// loses value), and the output is delivered exactly. Returns `None` if the
/// requested output would drain the destination reserve (or more).
pub fn swap_to_exact_out(
    destination_amount: u128,
    swap_source_amount: u128,
    swap_destination_amount: u128,
) -> Option<SwapWithoutFeesResult> {
    // Cannot take the entire (or more than the entire) destination reserve.
    ...
    // new_source = ceil(invariant / new_dest). Ceiling charges the swapper up,
    // preserving `new_source * new_dest >= invariant`. We keep the delivered
    // output exactly equal to `destination_amount`, so we ignore the adjusted
    // divisor that `checked_ceil_div` returns.
...
impl CurveCalculator for ConstantProductCurve {
    ...
    /// Constant product exact-output: solve x·y=k for the required input.
    fn swap_to_exact_out(
        &self,
        destination_amount: u128,
        swap_source_amount: u128,
        swap_destination_amount: u128,
...
mod tests {
    ...
    fn test_exact_out(
        ...
        // Input is rounded UP so the pool never loses value.
        assert_eq!(result.source_amount_swapped, expected_source_amount_swapped);
        let new_invariant = (swap_source_amount + result.source_amount_swapped)
            * (swap_destination_amount - result.destination_amount_swapped);
        assert!(new_invariant >= invariant);
    ...
    #[test]
    fn constant_product_swap_to_exact_out() {
        let curve = ConstantProductCurve;
        // x=1000 y=1000, want 100 out: new_dest=900, ceil(1_000_000/900)=1112, in=112
        test_exact_out(&curve, 100, 1000, 1000, 112);
        // want 1 out of a deep pool: new_dest=999_999, ceil(1e12/999_999)=1_000_002, in=2
        test_exact_out(&curve, 1, 1_000_000, 1_000_000, 2);
        // symmetric-ish: x=4_000_000 y=70_000_000_000, want 174_999 out
        // (inverse of the exact-in rounding case above)
        test_exact_out(&curve, 174_999, 4_000_000, 70_000_000_000, 10);
    }

    #[test]
    fn exact_out_cannot_drain_pool() {
        let curve = ConstantProductCurve;
        // asking for the entire (or more than the entire) destination reserve must fail
        assert!(curve
            .swap_to_exact_out(1000, 1000, 1000, TradeDirection::AtoB)
            .is_none());
        assert!(curve
            .swap_to_exact_out(1001, 1000, 1000, TradeDirection::AtoB)
            .is_none());
    }
