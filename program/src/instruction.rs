...
pub enum SwapInstruction {
    ...
    ///   Additive to Initialize; existing pools are unaffected.
    ///
    ...
    ///   8. `[writable]` Fee LP account PDA `[b"cp_fee", pool]` (created here; owner = pool).
    ///   9. `[writable]` Destination LP account PDA `[b"cp_dest", pool]` (created
    ///      here; owner = creator, so the creator holds the initial LP).
    ///   10. `[]` Token program.  11. `[]` System program.
